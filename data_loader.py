...
def load_train_data(dataset_name, scaler=None, train_shuffle=True, no_transform=False, verbose=False):
    ...
    if dataset_name  == 'BATADAL':
        try:
            df_train = pd.read_csv("ics-anomaly-detection-main/data/" + dataset_name + "/train_dataset.csv", 
        ...
        try:
            df_train = pd.read_csv("ics-anomaly-detection-main/data/" + dataset_name + "/WADI_train.csv")
        except FileNotFoundError:
...
def load_test_data(dataset_name, scaler=None, no_transform=False, verbose=False):
    ...
    if dataset_name  == 'BATADAL':
        try:
            df_test = pd.read_csv("ics-anomaly-detection-main/data/" + dataset_name + "/test_dataset_1.csv", 
                                  parse_dates = ['DATETIME'], dayfirst=True)
        except FileNotFoundError:
        ...
        try:
            df_test = pd.read_csv("ics-anomaly-detection-main/data/" + dataset_name + "/WADI_test.csv")
        except FileNotFoundError:
